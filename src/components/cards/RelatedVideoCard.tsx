...
const duration = moment.duration(diffInHours, 'hours');
  return (
    <Link href={`/watch/${post?.address}`} className="text-gray-400">
        <div className='flex flex-col  border-t-0 sm:max-w-sm w-full rounded-xl h-60 border border-fuchsia-900/50 relative my-2 p-1 font-mono '>
           <img src={`${IPFS_GATEWAY}${post?.metadata?.content?.content?.image}`} className="w-full h-[75%] object-cover rounded-md"  />
            <div>
               <h1 className='font-thin text-lg text-ellipsis mb-0'>{post?.metadata?.content?.content?.title&& shortenTxt(post?.metadata?.content?.content?.title, 23)}</h1>
               <h2 className='text-sm text-gray-400'>{post?.metadata?.content?.content?.title && shortenTxt(post?.address, 17)}</h2>
